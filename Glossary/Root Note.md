---
title: Root Note
type: concept
tags: [glossary, theory]
---

# Root Note

The **root note** (also called the **tonic**) is the "home" note a scale or chord is built from and named after. It's degree 1 of a [[Major Scale]] — and also degree 8, since the scale ends back on the same note one octave up.

Every other note in the scale is really just described *relative to* the root: "a whole step above the root," "a half step below the octave," and so on — see [[Major Scale]] for that formula. Change the root, and the whole scale shifts with it, but the shape relative to the root stays identical.

![[root-note.svg]]

C major's root is C. G major's root is G. F# major's root is F# — a black key works as a root just as well as a white one. Whatever note you start counting from *is* the root.

> [!info] Beyond scales
> Chords have root notes too (e.g. a C chord is built up from C as its root) — this concept comes back once [[PIX Series - Course Overview|chords]] are covered.

## Used in
- [[Major Scale]]
- [[Lesson 03 - Major Scales]]
